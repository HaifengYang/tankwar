...
public class Level {
    ...
    public static void loadLevel(Model gameModel) {
        ...
        //从上个关卡清除所有东西
        Arrays.fill(gameModel.actors, null);
    ...
    private static void addEnemySigns(Model gameModel) {
        int x = 527;
        int y = 90;
        for (int j = 0; j < 10; j++) {
            for (int i = 0; i < 2; i++) {
                gameModel.addEnemySigns(new EnemySign(x + i * 35, y + j * 30));
            }
